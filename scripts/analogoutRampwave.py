...
def parseCmdLine(args):
  ...
  parser = OptionParser();
  parser.add_option('-t', '--timeperiod' , dest='timeperiod' , type = float, default=def_t  , help='Time period of the ramp wave [sec] (default={:.2e})'.format(def_t));
  ...
  parser.add_option(      '--voltage1s'  , dest='voltage1s'  , type = str  , default=def_v1s, help='String of the voltage1(start voltage of the one ramp wave) [V] combined with \',\' for each channels. (ex, "0.0", "-1.0" for one channel, "0.0,0.0", "-1.0,0.0" for two channels) (default=\"{}\")'.format(def_v1s));
  parser.add_option(      '--voltage2s'  , dest='voltage2s'  , type = str  , default=def_v2s, help='String of the voltage2(end voltage of the one ramp wave) [V] combined with \',\' for each channels. (ex, "1.0", "2.0" for one channel, "1.0,1.0", "1.0,2.0" for two channels) (default=\"{}\")'.format(def_v2s));
  parser.add_option(      '--voltage1'   , dest='voltage1'   , type = float, default=None   , help='The voltage1(start voltage of the one ramp wave) [V] for all the channels. Usually this is None and VOLTAGE1S is used. (default=None)');
  parser.add_option(      '--voltage2'   , dest='voltage2'   , type = float, default=None   , help='The voltage2(end voltage of the one ramp wave) [V] for all the channels. Usually this is None and VOLTAGE2S is used. (default=None)');
  parser.add_option(      '--timeoffsets', dest='timeoffsets', type = str  , default=def_to , help='String of the start-time offset of the ramp wave [sec] cmbined with \',\' for each channels. (ex, "0.0", "-0.5e-6" for one channel, "0.0,0.5e-6", "0.5e+6,-0.5e+6" for two channels) (default=\"{}\")'.format(def_to));
...
def analogoutRampwave(
  timeperiod = 1.*usec,   # Time period of the ramp wave [sec]
  ...
  voltage1s  = [0.0,0.0], # Array of the voltage1(start voltage of the one ramp wave) [V] for each channels
  voltage2s  = [1.0,1.0], # Array of the voltage2(end   voltage of the one ramp wave) [V] for each channels
  timeoffsets= [0.0,0.0], # Start time offset of the ramp wave [sec] for each channels
  ...
  Out('time period = {} [usec]'.format(timeperiod/usec),0);
  Out('# of points in one period = {}'.format(npointsPeriod),0);
  buffer = [];
  for ch in range(g_nAnalogOutChannels) :
    ...
    Out('    start voltage of ramp wave = {} V'.format(voltage1s[chIndex]),0);
    Out('    end   voltage of ramp wave = {} V'.format(voltage2s[chIndex]),0);

    # Define waveform for one period
    waveform = np.linspace(voltage1s[chIndex], voltage2s[chIndex],npointsPeriod); # array size = npointsPeriod
  ...
  return ctx; # End of analogoutRampwave()
...
if __name__ == '__main__':
  ...
  ctx = analogoutRampwave(
      timeperiod = config.timeperiod, # Time period of the ramp wave [sec]
      ...
      voltage1s  = voltage1s,         # Array of the voltage1(start voltage of the one ramp wave) [V] for each channels
      voltage2s  = voltage2s,         # Array of the voltage2(end   voltage of the one ramp wave) [V] for each channels
      timeoffsets= timeoffsets,       # Start time offset of the ramp wave [sec] for each channels
